...
/*
    Optimized Approach : We can use binary search to find the minimum number of pages that can be allocated to a student
...
int findPages(vector<int> &arr, int k)
{
    ...
    int low = minimumPages, high = maximumPages;

    while (low <= high)
    {
        int mid = (low + high) / 2;
        int currentStudent = checkValidAllocation(arr, mid);
